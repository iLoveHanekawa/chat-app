...
function List(props: ListType) {
  ...
  return (
    <div ref = {props.listRef} className = ' py-8 z-10 w-full mt-10 md:mt-2 md:w-4/6 overflow-scroll h-nvh overflow-x-hidden right-0 top-0 absolute customScrollbar'>
        <ul className = 'flex flex-col py-1 px-3'>
            {props.msgs.map(({ isSent, msg, sentBy }: { isSent: boolean, msg: string, sentBy: string }, i: number) => {
                ...
                return <div key = {i} ref = {strayRef} className = 'flex mb-7 flex-col'>
                    ...
                    <div className = {`${isSent? 'self-end': 'self-start'} text-pink-300 text-xxs md:text-xs mt-1`}>{time.toString()}</div>
                </div>
            })}
        </ul>
    </div>
